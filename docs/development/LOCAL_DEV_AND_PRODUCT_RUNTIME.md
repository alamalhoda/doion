...
این سند منبع یکتای «کجا Django اجرا می‌شود و داده کجاست» است. قرارداد REST (`/api/v1/`) عوض نمی‌شود. نقشهٔ کامل توسعه/تست/استقرار هر دو لایه: [`DEVELOPMENT_TO_DEPLOY.md`](./DEVELOPMENT_TO_DEPLOY.md).
...
- استقرار از سورس با CLI چابکان یا workflow تأییدشدهٔ **CD Backend** در [`PRODUCTION_CHABOKAN_DEPLOY.md`](./PRODUCTION_CHABOKAN_DEPLOY.md) است؛ بدون `workflow_dispatch` مالک سرویس زنده عوض نمی‌شود. آموزش تگ: [`GIT_TAGS_AND_RELEASES.md`](./GIT_TAGS_AND_RELEASES.md). آموزش CD و شاخهٔ `product`: [`CHABOKAN_CD_AND_PRODUCT_BRANCH.md`](./CHABOKAN_CD_AND_PRODUCT_BRANCH.md)
