...
Current ship path for `chequeyar-back` is still `chabok deploy` from `backend/` (source). A GitHub **Release** workflow can push tagged images to GHCR (`ghcr.io/alamalhoda/doion-api` and `ghcr.io/alamalhoda/chequeyar-front`); that does not change the live Chabokan services. Owner-approved CD is a later step.
